...
BEGIN
	...
	BEGIN
		...
		-- Ver si ya hay una nota de crédito para este detalle
		IF EXISTS (SELECT 1 FROM Venta.NotaDeCredito WHERE idDetalleVenta = @idDetalleVenta)
		BEGIN
			RAISERROR ('Ya hay una nota de credito para este detalle de venta.', 16, 1);
			RETURN;
		END
		...
		IF @cambioProducto = 0
		BEGIN
			INSERT INTO Venta.NotaDeCredito(idDetalleVenta, comprobante, motivo, fecha, hora, monto)
			VALUES (@idDetalleVenta, @comprobante, @motivo, CAST(GETDATE() AS DATE), CAST(GETDATE() AS TIME), @precioUnitario * @cantidadVendida)
		END
		ELSE
		BEGIN
			INSERT INTO Venta.NotaDeCredito(idDetalleVenta, comprobante, motivo, fecha, hora, idProductoCambio)
